...
namespace Common {
#ifdef __FILE_NAME__
#define FACE_AUTH_LOG_FILE __FILE_NAME__
#else
#define FACE_AUTH_LOG_FILE (__builtin_strrchr(__FILE__, '/') ? __builtin_strrchr(__FILE__, '/') + 1 : __FILE__)
#endif
...
#define LOG_DOMAIN 0xD002406

#ifdef LOG_TAG
#undef LOG_TAG
#endif

#define FACE_DEBUG(...) HILOG_DEBUG(LOG_CORE, __VA_ARGS__)
#define FACE_INFO(...) HILOG_INFO(LOG_CORE, __VA_ARGS__)
#define FACE_WARN(...) HILOG_WARN(LOG_CORE, __VA_ARGS__)
