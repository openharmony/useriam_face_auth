 ...
 */
#ifndef FACE_AUTH_MANAGER_ENROLL_TEST_H
#define FACE_AUTH_MANAGER_ENROLL_TEST_H
#include <iostream>
#include <fstream>
#include <thread>
...
public:
    ...
    void OnCallbackEvent(uint64_t reqId, int32_t type, int32_t code, int32_t errorCode, Bundle &bundle)
    {
        ...
        for (std::vector<uint8_t>::const_iterator iter = bundle.authtoken.begin(); iter != bundle.authtoken.end();
             ++iter) {
            FACEAUTH_LABEL_LOGI("token is %{public}d", *iter);
        }

        if (code_ == CODE_CALLBACK_RESULT || code_ == CODE_CALLBACK_CANCEL) {
            isCallback_ = true;
        }
        FACEAUTH_LABEL_LOGI("token is %{public}d", bundle.authtoken.size());
...
class FaceEnrollManagerTest : public testing::Test {
public:
...
#endif // FACE_AUTH_MANAGER_ENROLL_TEST_H