...
{
    IAM_LOGI("fsm %{public}s created for %{public}zu transitions", name_.c_str(), transitionMap_.size());
}

FiniteStateMachineImpl::~FiniteStateMachineImpl()
{
    pendingEvents_.Clear();
    IAM_LOGI("fsm %{public}s destroyed for %{public}zu transitions", name_.c_str(), transitionMap_.size());
}
